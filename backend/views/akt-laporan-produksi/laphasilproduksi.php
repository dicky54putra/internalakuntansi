...
<head>
    ...
    <title>Rekap Hasil Produksi</title>
    <style>
        body {
            margin: 0;
            padding: 0;
...
<body>
    ...
    <h2 class="title-lap"> Rekap Hasil Produksi </h2>
    ...
    <table width="100%">
        ...
        <?php
        $bahan = Yii::$app->db->createCommand("SELECT SUM(qty) AS qty , id_item_stok, tanggal FROM ( SELECT akt_produksi_bom_detail_hp.*, akt_produksi_bom.tanggal FROM akt_produksi_bom LEFT JOIN akt_produksi_bom_detail_hp ON akt_produksi_bom_detail_hp.id_produksi_bom = akt_produksi_bom.id_produksi_bom UNION SELECT akt_produksi_manual_detail_hp.*,akt_produksi_manual.tanggal FROM akt_produksi_manual LEFT JOIN akt_produksi_manual_detail_hp ON akt_produksi_manual_detail_hp.id_produksi_manual = akt_produksi_manual.id_produksi_manual ) AS Hasil WHERE tanggal BETWEEN '$tanggal_awal' AND '$tanggal_akhir' GROUP BY id_item_stok")->query();
        $no = 1;
        $total = 0;
        foreach ($bahan as $b) {
            $total += $b['qty'];
